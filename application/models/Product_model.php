...
class Product_model extends Core_Model
{
	...
	public function build_query_unlocated($args = [])
	{
		$default_args = [
			'products.status' => [ 1 ],
			'products.is_draft' => [ 0 ],
		...
		$params = array_replace_recursive($default_args, $args);

		$this->db->join('users', 'products.user_id = users.id');
		$this->db->select('products.*, users.username as user_username, users.shop_name as shop_name, users.role as user_role, users.slug as user_slug');
		$this->db->where('users.banned', 0);
		$this->db->where_in('products.status', (array) $params['products.status']);
		$this->db->where('products.visibility', 1);
		$this->db->where_in('products.is_draft', (array) $params['products.is_draft']);
		$this->db->where('products.is_deleted', 0);
	...
	public function add_ongkir()
	{
		...
		foreach ($this->languages as $language) {           
			...
			$this->db->insert('product_ongkir', $data);
		}
	}

	//edit ongkir
    public function edit_ongkir($common_id)
    {
        foreach ($this->languages as $language) {
            $data = array(
				'state_id' => $this->input->post('state_id', true),
            ...
            $this->db->update('product_ongkir', $data);
        }
	}
    ...
    //delete variation
    public function delete_ongkir($common_id)
    {
        $ongkir = $this->get_ongkir_by_common_id($common_id);
        if (!empty($ongkir->id)) {
			$this->db->where('id', $ongkir->id);
			$this->db->delete('product_ongkir');
		}
        return false;
	}

	public function _ongkir_by_provinsi($product_id, $user) {
		$this->db->where('product_id', $product_id);
		$this->db->where('state_id', $user->shipping_state);						
		$this->db->where('city_id', '');						
		$this->db->where('address', '');	
		$query = $this->db->get('product_ongkir');			
		return $query->row();		
	}

	public function _ongkir_by_kota($product_id, $user) {
		$this->db->where('product_id', $product_id);
		$this->db->where('state_id', $user->shipping_state);						
		$this->db->where('city_id', $user->shipping_city);						
		$this->db->where('address', '');	
		$query = $this->db->get('product_ongkir');			
		return $query->row();		
	}

	public function _ongkir_by_kecamatan($product_id, $user) {
		$this->db->where('product_id', $product_id);
		$this->db->where('state_id', $user->shipping_state);						
		$this->db->where('city_id', $user->shipping_city);						
		$this->db->like('address', $user->shipping_kecamatan);		
		$query = $this->db->get('product_ongkir');			
		return $query->row();		
	}
	
	// list ongkir 
	public function get_list_ongkir_by_user($product_id, $user)
	{
		$provinsi = $user->shipping_state;
		$kota = $user->shipping_city;
		$kec = $user->shipping_kecamatan;

		$q = $this->_ongkir_by_kecamatan($product_id, $user);
		if(empty($q)) {
			$q = $this->_ongkir_by_kota($product_id, $user);
			if(empty($q)) {
				$q = $this->_ongkir_by_provinsi($product_id, $user);
			}
		}
	...
	// list ongkir 
	public function get_list_ongkir_by_select($product_id, $provinsi=null, $kota=null, $kecamatan=null)
	{

		$this->db->where('product_id', $product_id);
		$this->db->where('state_id', $provinsi);						
		$this->db->where('city_id', $kota);						
		$this->db->like('address', $kecamatan);		
		$query = $this->db->get('product_ongkir');	
		$q = $query->row();	
		if(empty($q)) {
	...
	//get product ongkir
	public function get_product_ongkirs($product_id)
	{
		$product_id = clean_number($product_id);
		$this->db->where('product_id', $product_id);
		$query = $this->db->get('product_ongkir');
    ...
    //get ongkir by common id
    public function get_ongkir_by_common_id($common_id)
    {
        $this->db->where('common_id', $common_id);
        $query = $this->db->get('product_ongkir');
        return $query->row();
