...
class Category_model extends CI_Model
{
	...
	public function input_values()
	{
		$data = array(
			...
			'show_icon' => $this->input->post('show_icon', true),
	...
	public function update_category($id)
	{
		...
		if (!empty($temp_path)) {
			$data["image_1"] = $this->upload_model->category_image_upload($temp_path, 420, 312);
			$data["image_2"] = $this->upload_model->category_image_upload($temp_path, 440, 541);
			$data["image_3"] = $this->upload_model->category_image_upload($temp_path, 1200, 800);
			$this->upload_model->delete_temp_image($temp_path);
			$category = $this->get_category($id);
			$data["storage"] = "local";
			...
			if ($this->storage_settings->storage == "aws_s3") {
				...
				delete_file_from_server($data["image_2"]);
				//move image 2
				$this->aws_model->put_category_object($data["image_3"], FCPATH . $data["image_3"]);
				delete_file_from_server($data["image_3"]);
			}
			//delete old images
			if ($category->storage == "aws_s3") {
				...
				$this->aws_model->delete_category_object($category->image_3);
			} else {
				delete_file_from_server($category->image_1);
				delete_file_from_server($category->image_2);
				delete_file_from_server($category->image_3);
			}
		}

		$temp_path = $this->upload_model->upload_temp_image('file_icon');
		if (!empty($temp_path)) {
			$data["icon"] = $this->upload_model->icon_image_upload($temp_path, 100, 100);
			$this->upload_model->delete_temp_image($temp_path);
			$category = $this->get_category($id);
