...
class Earnings_admin_model extends CI_Model
{
    ...
    public function filter_deposit()
    {
        ...
        $data['q'] = trim($data['q']);
        if (!empty($data['q'])) {
            $this->db->where('deposit.user_id', $data['q']);
        }
    }
    ...
    public function get_completed_deposit_count()
    {
        $this->filter_deposit();
    ...
    public function get_paginated_completed_deposit($per_page, $offset)
    {
        $this->filter_deposit();
        $this->db->where('deposit.status', 1);
    ...
    public function get_deposit_requests_count()
    {
        $this->filter_deposit();
    ...
    //complete payout
    public function complete_deposit($payout_id, $user_id, $amount)
    {
        $payout_id = clean_number($payout_id);
        $user_id = clean_number($user_id);
        $data = array(
        ...
        $this->db->where('id', $payout_id);
        $update = $this->db->update('deposit', $data);
        if ($update) {
            return $this->increase_user_balance($user_id, $amount);
        }
    ...
    public function increase_user_balance($user_id, $amount)
    {
        $user_id = clean_number($user_id);
        $user = $this->auth_model->get_user($user_id);
        if (!empty($user)) {
