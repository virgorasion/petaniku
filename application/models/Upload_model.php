...
class Upload_model extends CI_Model
{
	...
	//foto ktp & diri
	public function foto_toko_default($path)
	{
		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 85;
	...
	//icon image upload
	public function icon_image_upload($path, $width, $height)
	{
		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 85;
			$image->crop($width, $height, true);
			$new_path = 'uploads/category/category_' . $width . '-' . $height . '_' . generate_unique_id() . '.png';
	...
	//paket image upload
	public function paket_upload($path)
	{
		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 85;
			$image->crop(400, 200, true);
			$new_path = 'uploads/product/paket_' . generate_unique_id() . '.jpg';
			$image->save(FCPATH . $new_path, IMAGETYPE_JPEG);
			return $new_path;
		} catch (ImageResizeException $e) {
			return null;
	...
	public function video_upload($file_name)
	{
		...
		try {
			if(!$this->upload->do_upload($file_name)){
				die(json_encode($this->upload->display_errors()));
			}
