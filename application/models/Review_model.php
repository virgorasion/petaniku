...
class Review_model extends CI_Model
{
	...
	public function get_user_reviews($user_id)
	{
		...
		$this->db->join('users', 'users.id = reviews.user_id');		
		$this->db->join('products', 'products.id = reviews.product_id');
		$this->db->select('reviews.*, products.slug as product_slug, products.title,  users.username as user_username, users.slug as user_slug');
		$this->db->where('products.user_id', $user_id);
		$this->db->order_by('reviews.created_at', 'DESC');
		$query = $this->db->get('reviews');
		return $query->result();
    ...
    public function get_user_rating($seller_id)
    {
        ...
        $reviews = $this->get_user_reviews($seller_id);
        if (!empty($reviews)) {
            $count = count($reviews);
            $total = 0;
