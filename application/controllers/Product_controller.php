...
class Product_controller extends Home_Core_Controller
{
	...
	public function start_selling_post()
	{
		...
		$temp_path = $this->upload_model->upload_temp_image('foto_ktp');
		if (!empty($temp_path)) {
			$foto_ktp = $this->upload_model->foto_toko_default($temp_path);
			$this->upload_model->delete_temp_image($temp_path);
		}

		$temp_path = $this->upload_model->upload_temp_image('foto_selfi');
		if (!empty($temp_path)) {
			$foto_selfi = $this->upload_model->foto_toko_default($temp_path);
			$this->upload_model->delete_temp_image($temp_path);
		}
	...
	public function add_product_post()
	{
		...
		if (empty($this->session->userdata('modesy_product_images'))) {
			$this->session->set_flashdata('errors', '<p>Gambar wajib diisi!</p>');
			redirect($this->agent->referrer());
		} else {
			//add product
	...
	public function edit_product_details($id)
	{
		...
		if ($data["product"]->country_id == 0) {
			...
			$data["cities"] = $this->locationid_model->get_cities_by_state($data["product"]->state_id);
		}
		if ($data["product"]->country_id == 0) {
			$data['kecamatan'] = [];
		} else {
			$data['kecamatan'] = $this->locationid_model->get_kecamatan_by_city($data["product"]->city_id);
		}
		...
		$data["product_ongkirs"] = $this->product_model->get_product_ongkirs($data["product"]->id);
	...
	*/
	...
	//add product ongkir
	public function add_product_ongkir()
	{
		if ($this->auth_check) {
			$product_id = $this->input->post('product_id', true);
			$this->product_model->add_ongkir();
			$data["ongkirs"] = $this->product_model->get_product_ongkirs($product_id);			
			$this->load->view('product/ongkir/_list_ongkir', $data);											
		}
	}

	//edit product ongkir
	public function edit_product_ongkir()
	{
		if ($this->auth_check) {
			...
			$data["ongkir"] = $this->product_model->get_ongkir_by_common_id($common_id);
			$data["states"] = $this->locationid_model->get_states_by_country($this->auth_user->country_id);			
			$data["cities"] = $this->locationid_model->get_cities_by_state($data["ongkir"]->state_id);			
			$data["kecamatan"] = $this->locationid_model->get_kecamatan_by_city($data["ongkir"]->city_id);			
			$this->load->view('product/ongkir/_form_ongkir_edit', $data);
		}
	}

	//edit product ongkir
	public function edit_product_ongkir_post()
	{
		if ($this->auth_check) {
			$common_id = $this->input->post('common_id', true);
			$product_id = $this->input->post('product_id', true);
			$this->product_model->edit_ongkir($common_id);
	...
	public function delete_product_ongkir()
	{
		if ($this->auth_check) {
			$common_id = $this->input->post('common_id', true);
			$product_id = $this->input->post('product_id', true);
			$this->product_model->delete_ongkir($common_id);
	...
	*/
	...
	public function add_product_variation()
	{
		if ($this->auth_check) {
			...
			$this->load->view('product/paket/_response_paket', $data);											
		}
	}
	...
	public function edit_product_variation()
	{
		if ($this->auth_check) {
			...
			$this->load->view('product/paket/_form_paket_edit', $data);
		}
	}
	...
	public function loadmore_homepage()
	{
		...
		if(!empty($page)){
			$offset = ceil($page * $perpage);
			$query = $this->db->order_by('created_at', 'DESC')->get('products',$perpage,$offset)->result();
			$data['latest_products'] = $query;
	  
		}else{
			$query = $this->db->order_by('created_at', 'DESC')->get('products',$perpage)->result();			
			$data['latest_products'] = $query;
		}
		$response['view'] = $this->load->view('product/_load_more', $data, true);		
	...
	{
		$city_id = $this->input->post('cities_id', true);
		$kecamatan = $this->locationid_model->get_kecamatan_by_city($city_id);
	...
	{
		$country_id = $this->input->post('country_id', true);
		$states = $this->locationid_model->get_states_by_country($country_id);
	...
	{
		$state_id = $this->input->post('state_id', true);
		$cities = $this->locationid_model->get_cities_by_state($state_id);
		foreach ($cities as $item) {
			echo '<option value="' . $item->id . '">' . $item->name . '</option>';
		}
	}

	public function get_ongkir($pid=null)
	{
		...
		$provinsi = $this->input->post('state_id', true);
		$kota = $this->input->post('city_id', true);

		if($filter == 'kota') {
			$this->db->where('product_id', $pid);
			$this->db->where('state_id', $provinsi);
			$q = $this->db->get('product_ongkir');						
			$data = $q->result();
			$cityid = [];
			foreach((array) $data as $r) {
				array_push($cityid, $r->city_id);
			}

			$cities = $this->locationid_model->get_cities_in_array($cityid);			
			foreach ((array) $cities as $item) {
				echo '<option value="' . $item->id . '">' . $item->name . '</option>';
			}
		}
		if($filter == 'kecamatan') {
			...
			$kecamatanid = [];
			foreach((array) $data as $r) {
				array_push($kecamatanid, $r->address);
			}

			$kecamatan = $this->locationid_model->get_kecamatan_in_array($kecamatanid);			
			foreach ((array) $kecamatan as $item) {
				echo '<option value="' . $item->id . '">' . $item->name . '</option>';
			}
		}
	}
