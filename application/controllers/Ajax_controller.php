	...
	*/
	...
	public function search_location()
	{
		...
		if (!empty($input_value)) {
			...
			// 	}
			// }
			$states = $this->locationid_model->search_states($input_value);
			if (!empty($states)) {
				$data['result'] = 1;
				foreach ($states as $state) {
					$response .= '<li><a href="javascript:void(0)" data-country="' . $state->country_id . '" data-state="' . $state->id . '"><i class="icon-map-marker"></i>' . replace_kab($state->name) . ', ' . $state->country_name . '</a></li>';
				}
			}
			$cities = $this->locationid_model->search_cities($input_value);
			if (!empty($cities)) {
				$data['result'] = 1;
				foreach ($cities as $city) {
					$response .= '<li><a href="javascript:void(0)" data-country="' . $city->country_id . '" data-state="' . $city->state_id . '" data-city="' . $city->id . '"><i class="icon-map-marker"></i>' . replace_kab($city->name) . ', ' . replace_kab($city->state_name) . ', ' . $city->country_name . '</a></li>';
				}
			}
			$response .= '</ul>';
